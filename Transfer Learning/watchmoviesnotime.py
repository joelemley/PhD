...
window=1 # Number of frames in sliding window.
...
#test= train[train['subject'] == np.unique(train['subject'])[0]]
# a=test[test['classname'] =='c1']
# t=a.values[0]
#
# for t in a.values:
...
for subj in np.unique(train['subject'])[20:]:
    ...
    for t in imagem.values:
        if (oldexpression!= [int(t[1][1:])]):
            ...
            gray_image = cv2.resize(gray_image,(80,60))
            frames.append(np.asarray(gray_image))
            print np.shape(np.asarray(gray_image))
        else:
            img = cv2.imread('imgs/train/' + t[1] + '/' + t[2], 3)
            gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            gray_image = cv2.resize(gray_image,(80,60),interpolation=cv2.INTER_AREA)
...
for clip in clips: # expand clips with sliding window.
    for i in xrange(0,len(clip)-window):
...
np.save('valclipsnotime.np',X)
np.save('vallabelsnotime.np',Y)
