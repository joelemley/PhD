...
def prepareData(person):
    ...
    for sampleid in xrange(0,len(data)):
        ...
        if(whicheye=="left"):
            ...
            labels.append(lbl)
            left.append(np.reshape(image,(1,36,60)))
        else:
            ...
            labels.append(lbl)
            right.append(np.reshape(image, (1, 36, 60)))

    return labels,right,left

import matplotlib.pyplot as plt
import matplotlib.image as mpimg
import numpy as np

labels,right,left=prepareData("00")

fig = plt.figure(figsize=(5, 5))
sub = fig.add_subplot(3, 1, 0 + 1)
sub.imshow(np.squeeze(right[0]), interpolation='nearest')
sub = fig.add_subplot(3, 1, 1 + 1)
sub.imshow(np.flip(np.squeeze(right[0]),1), interpolation='nearest')

sub = fig.add_subplot(3, 1,  1+2)
sub.imshow(np.squeeze(left[0]), interpolation='nearest')
fig.show()
plt.show()
