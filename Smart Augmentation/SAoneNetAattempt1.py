...
networkAi = lasagne.layers.InputLayer(shape=(None , netAnumInputs*3 , None , None) , name='networkAi')
...
networkAOut = lasagne.layers.Conv2DLayer(netAL4,num_filters=3,filter_size=(3,3),pad='same',name='networkAOut')
...
for epoch in range(num_epochs):
    ...
    if validationError < validationErrorBest:
        ...
        with open('netBestSAoneNetAattempt1netAerr.pickle' , 'wb') as handle:
            print('saving the modelA....')
            pickle.dump(networkAOut , handle)
        with open('netBestSAoneNetAattempt1netBerr.pickle' , 'wb') as handle:
    ...
    plt.show()
    plt.pause(0.0001)
    scipy.io.savemat('LossesSAoneNetAattempt1.mat',mdict={'TrainLoss':TRAINLOSS , 'ValidationLoss':VALIDATIONLOSS , 'TrainLossB':TRAINERRORB})
